...
///ova e so napravena funkcija za mapiranje i filtriranje
function mapArray(arr,mapFnc){
    let resultArray=[];
    for(let item of arr){
        resultArray.push(mapFnc(item));
    }
    return resultArray;
}

let mappedStudentsFullNames=mapArray(students,(student=>{
    return {
        fullName:student.firstName + " " + student.lastName,
        age:student.age
    }
}));
console.log(mappedStudentsFullNames)
...
let studentsWithGradeHigherThanThree1 = filterArray(students,student=>student.averageGrade>3);
console.log(studentsWithGradeHigherThanThree1);
//so koristenje na map i filter
// All students with an average grade higher than 3
let studentsWithGradeHigherThanThree2=students.filter(grade=>grade.averageGrade>3);
console.log(studentsWithGradeHigherThanThree2)
// All female student names with an average grade of 5
let femaleStudentsWithGradeofFive=students.filter(grade=>grade.averageGrade===5).filter(female=>female.gender==="Female");
console.log(femaleStudentsWithGradeofFive);
// All male student full names who live in Skopje and are over 18 years old
let maleStudentsFromSkopjeAndOver18=students.filter(town=>town.city==="Skopje").filter(age=>age.age>=18).filter(gender=>gender.gender==="Male").map(ime=>{
    return{
        fullName:`${ime.firstName} ${ime.lastName}`,
        age:ime.age,
    }
})
console.log(maleStudentsFromSkopjeAndOver18)

// The average grades of all female students over the age of 24

let femaleStudentsOver24=students.filter(age=>age.age>24).filter(gender=>gender.gender==="Female").map(info=>{
    return{
        fullName:`${info.firstName} ${info.lastName}`,
        averageGrade:info.averageGrade,
    }
})
console.log(femaleStudentsOver24)

let avgGradeofFemaleStudentsOver24=femaleStudentsOver24.reduce((total,next)=> total+next.averageGrade,0)/femaleStudentsOver24.length;

console.log(avgGradeofFemaleStudentsOver24)

// All male students with a name starting with B and average grade over 2

let maleStudentsWithGradeOver2=students.filter(gender=>gender.gender==="Male").filter(grade=>grade.averageGrade>2);
console.log(maleStudentsWithGradeOver2)

// let maleStudentsWithFirstLetterOfTheNameBandWithGradeOver2=maleStudentsWithGradeOver2.filter((x)=> x.startsWith("B"));
// console.log(maleStudentsWithFirstLetterOfTheNameBandWithGradeOver2)

//ne mozam so filter.startsWith da ja resam